...
#camels_joined = mutate(camels_joined, CONUS = case_when(
  #DIVISION != "APPALACHIAN HIGHLANDS" ~ "CONUS", TRUE ~ "CONUS"))

head(camels_joined)
  
app_gauges = filter(camels_joined, DIVISION == "APPALACHIAN HIGHLANDS" & PROVINCE != "NEW ENGLAND" & PROVINCE != "ADIRONDACK" & PROVINCE != "ST. LAWRENCE VALLEY")
...
Conus_Maps <- tmap_arrange(USgages_BFI2, USgages_Qmean, USgages_Q5, USgages_Q95, nrow = 2, ncol = 2)
Conus_Maps
...
dataset1 <- as.numeric(camels_joined$RBI, na.rm = TRUE)
dataset2 <- as.numeric(app_gauges$RBI, na.rm = TRUE)

# Define function to compute Mann-Whitney U statistic
mann_whitney_u <- function(data1, data2) {
  wilcox.test(data1, data2, alternative = "two.sided")$statistic
}

# Define bootstrapping function
bootstrap_func <- function(data1, data2, indices) {
  dataset1_sample <- data1[indices]
  dataset2_sample <- data2[sample.int(length(data2), replace = TRUE)]
  mann_whitney_u(dataset1_sample, dataset2_sample)
}

# Perform bootstrapping
bootstrap_results <- boot(
  data = list(dataset1 = dataset1, dataset2 = dataset2),
  statistic = bootstrap_func,
  R = 1000  # Number of bootstrap samples
)

# Calculate confidence interval for the statistic
boot_ci <- boot.ci(bootstrap_results, type = "basic")$normal
