...
setwd("C:/Users/lfink/Desktop/Hydro Research 2022 2023/Exports")
...
setwd("C:/Users/lfink/Desktop/Hydro Research 2022 2023/CAMELSII Data/Watershed characteristics")

Soils <- read_csv("conterm_soils.txt") %>%
  filter(STAID %in% sitenos)

Landcover <- full_join(read_csv("conterm_lc06_basin.txt"), read_csv("conterm_lc06_rip800.txt"), by = "STAID") %>% 
  filter(STAID %in% sitenos)

Topography <- read_csv("conterm_topo.txt") %>%
  filter(STAID %in% sitenos)

WSfactors <- full_join(read_csv("conterm_geology.txt"), read_csv("conterm_hydro.txt"), by = "STAID") %>%
  full_join(read_csv("conterm_climate.txt"), by = "STAID") %>%
  full_join(read_csv("conterm_basinid.txt"), by = "STAID") %>%
  filter(STAID %in% sitenos)
```
